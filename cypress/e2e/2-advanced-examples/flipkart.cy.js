describe("test sue of fipkart", () => {
  it("testcase for flipkart", () => {
    ...
    cy.get('a[rel="noopener noreferrer"]')
      .first()
    ...
    cy.get(
      '[id="container"]>div>div:nth-child(3)>div:nth-child(1)>div:nth-child(1)>div:nth-child(2)>div>ul>li:nth-child(1)'
    ).click();
  });
  ...
  it("Add to cart example", () => {
    ...
    cy.get('h3[class="price"]').each((price) => {
      ...
      const priceWithoutCurrency = priceText
        .replace("₹", "")
        .replace(",", "")
        .replace(",", "")
        .trim();
    ...
    cy.get('div[class="cart_content"]>div:nth-child(3)>h3>b').then(
      (totalamount) => {
        var totalprice = totalamount.text();
        var withoutrupie = totalprice
          .replace("₹", "")
          .replace(",", "")
          .replace(",", "");
      ...
      .then(() => {
        ...
        cy.get('div[class="cart_content"]>div:nth-child(3)>h3>b').then(
          (totalAmount) => {
            ...
            const priceWithoutCurrency = totalPriceText
              .replace("₹", "")
              .replace(/,/g, "")
              .trim();
